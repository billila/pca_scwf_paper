...
# data ####
data <- as.Seurat(sce_m, counts = "counts", data = NULL, assay = NULL)
data
...
data <- subset(data, subset = nFeature_originalexp > 200 & nFeature_originalexp < 2500  & percent.mt < 5 &
                 nCount_originalexp < 4000)
end_time <- Sys.time()
time_elapsed <- end_time - start_time
print(paste("Time Elapsed:", time_elapsed))
...
save(list = "hvg", file = "/mnt/spca/pipeline_sc/cb/seurat/cb_seurat_hvg.RData")
...
library(mclust)
ARI <- adjustedRandIndex((data$celltype), data$seurat_clusters)
